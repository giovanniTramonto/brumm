...
- **Kind-Status**: Neu angelegte Kinder sind nie automatisch aktiv (`isActive: false`). Reihenfolge: `Ausstehend` (offener Invite) → `Bestätigt` (Invite geklickt oder Superuser ist Guardian) → `Aktiv` (freigeschaltet) / `Abgemeldet` (deaktiviert)
