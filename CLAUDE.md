...
- **isOwnChild**: `GET /api/ini/{slug}/members` gibt für jeden Member `isOwnChild: boolean` zurück — `true` wenn eine Guardian-Email des Kindes mit einer Email des eingeloggten Users übereinstimmt (für MEMBER: eigene Guardian-Emails; für andere Rollen immer `false`). Der Members-Endpoint filtert **nicht** mehr serverseitig — alle Member werden zurückgegeben, die Filterung auf eigene Kinder erfolgt clientseitig (`DashboardMyKids`, `members/index.vue`). Der Detail-Endpoint (`GET /api/ini/{slug}/members/{id}`) berechnet `isOwnChild` zusätzlich für SUPERUSER (eigene Guardian-Email) und gibt es separat zurück — steuert Upload- und „Als gelesen markieren"-Buttons im `canManageMembers`-Template-Block.
