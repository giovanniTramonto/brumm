...
- @aws-sdk/client-s3 + @aws-sdk/s3-request-presigner (S3-kompatibler Datei-Storage, optional)
...
- **Storage pro Verein**: Beim Onboarding verbindet der SUPERUSER seinen Google-Account via OAuth 2.0 und gibt die **ID einer Geteilten Ablage** (Pflichtfeld) ein. Alle Vereinsdaten werden ausschließlich in dieser Geteilten Ablage angelegt. Das OAuth-Token wird in `Club.oauthToken` (Datenbank) gespeichert. Kein globaler Service Account, kein GCP-Setup durch den Verein nötig. Die Shared Drive ID wird als `parentId` durch den OAuth-State (`/api/ini/{slug}/auth/google` → Callback → `setupClubStorage` → `createRootFolderStructure`) durchgereicht.
- **Datentrennung**: Die zentrale Datenbank speichert technische/Auth-Daten (`id`, `clubId`, `role`, `status`, `storageId`, `deactivatedAt`, `hasSubmittedDocuments`) sowie Vereins-Konfiguration (`membershipFee`). Alle persönlichen Mitgliederdaten (`firstName`, `lastName`, `birthDate`, `emails`, `phone1`, `phone2`, `groupId`, `surcharges`, `careType`, `contractEnd`, etc.) leben standardmäßig in Google Sheets (Spalten: A=userId, B=storageRef, C=firstName, D=lastName, E=birthDate, F=guardian1Name, G=guardian2Name, H=email1, I=email2, J=groupId, K=contractEnd, L=phone1, M=phone2, N=surcharges, O=careType, P=lastEditedAt, Q=lastEditedBy, R=address). Optional kann ein Verein auf ein eigenes PostgreSQL-Backend umstellen (`ClubDatabase.type = POSTGRES`) — dann laufen alle Datenzugriffe über den per-Club Postgres-Client (`getClubDb`)
- **Pluggable Storage-Backends**: Jeder Verein kann unabhängig zwischen zwei Backends wählen — für Daten (`ClubDatabase`: `GOOGLE_SHEETS` oder `POSTGRES`) und für Dateien (`ClubFileStorage`: `GOOGLE_DRIVE` oder `S3`). Die Konfiguration wird verschlüsselt in der zentralen DB gespeichert (`encryptedDsn` / `encryptedConfig` via AES-256-GCM). `server/utils/memberData.ts`, `managerData.ts`, `teamData.ts`, `groupData.ts` prüfen via `getClubDbType` jeweils welches Backend aktiv ist und verzweigen entsprechend. Dateizugriffe (Upload, Download, Löschen) verzweigen über `getClubStorageType` zwischen Google Drive (`server/utils/storage/googleDrive.ts`) und S3 (`server/utils/storage/s3/files.ts`). Transfer-Endpunkte (`/api/ini/{slug}/settings/database/transfer`, `/api/ini/{slug}/settings/file-storage/transfer`) kopieren alle Daten einmalig vom alten ins neue Backend und schalten danach atomar um. S3-Keys werden als base64url-kodierte Strings in URLs und in `DocumentTemplate.s3Key` / `Document.s3Key` / `MemberDocument.s3Key` gespeichert
...
- **Vertragsunterlagen (DocumentTemplates/contract-templates)**: Drei Typen: `read` (Nur lesen — Eltern bestätigen gelesen), `upload` (Ausfüllen — Vorlage herunterladen, ausfüllen, hochladen), `submit` (Einreichen — beliebiges Dokument hochladen, keine Vorlage nötig). Aktive/abgemeldete Kinder sehen nur Vorlagen mit vorhandener Einreichung. „Als gelesen markieren" ist einmalig und speichert eine Kopie der Vorlagendatei im Storage des Kindes (Drive-Ordner oder S3-Prefix). Sobald ein Kind aktiv ist, sind Vertragsunterlagen schreibgeschützt (kein Löschen mehr möglich). Vor der Aktivierung können Dateien im Kein-Invite-Workflow gelöscht werden (`DELETE /api/ini/{slug}/members/{id}/documents/{fileId}`, nur `canManageMembers`). Verwaltung der Vorlagen via `canManageMembers` (SUPERUSER + isMemberManager). URL: `/ini/{slug}/contract-templates`. Datei-Upload: max. 5 MB (konfiguriert in `utils/config.ts` als `MAX_UPLOAD_SIZE_BYTES`/`MAX_UPLOAD_SIZE_LABEL`, clientseitig und serverseitig geprüft). Hochgeladener Dateiname wird neben dem Upload-Button angezeigt.
- **Einreichen-Flow**: MEMBER klickt „Einreichen" (Button in der Form-Action-Zeile neben „Speichern", nur bei `status === 'REGISTERED' && hasInvite && !submitted`) sobald alle Vertragsunterlagen vollständig sind. Confirm-Dialog zeigt Hinweis dass die Kita eine Benachrichtigung erhält. `User.hasSubmittedDocuments` wird auf `true` gesetzt (Datenbank). Benachrichtigung geht an: MANAGERs mit `isMemberManager = true` (Emails aus Sheets), oder – falls keine solchen existieren – an den SUPERUSER. Der „Aktivieren"-Button ist erst aktiv wenn `hasSubmittedDocuments = true` (und `member.hasInvite = true`); neben dem Titel „Vertragsunterlagen" erscheint ein „Noch nicht eingereicht"-Hinweis solange noch nicht eingereicht wurde. Im **Kein-Invite-Workflow** ist „Aktivieren" immer aktiv (unabhängig von `hasSubmittedDocuments`). Sind keine Vertragsvorlagen konfiguriert, erscheint ein oranger Hinweis (für MEMBER: „Die Kita hat noch keine Vertragsunterlagen hinterlegt."; für `canManageMembers`: mit Link zu `/contract-templates`).
...
- **filteredDocuments**: Computed in `pages/ini/[slug]/members/[id]/index.vue` — Dateien aus `documents/` (Drive oder S3), die nicht bereits als Template-Einreichung (über `MemberDocument`-Eintrag) erfasst sind. Wird in der `canManageMembers`-Sektion nur angezeigt wenn `member.status === 'REGISTERED' && member.hasInvite` (verhindert Doppeldarstellung nach Aktivierung, da der ACTIVE-Block ohnehin alle Dokumente zeigt).
...
- **Wall / Aktuell** (`/ini/{slug}/wall`): SUPERUSER und MANAGER können Einträge vom Typ `document` (Datei-Upload → Google Drive oder S3) oder `link` (externe URL → nur DB) anlegen, sortieren (Drag & Drop) und löschen. Datenbank-Tabelle `Document` (`clubId`-isoliert) speichert `name`, `order`, `type` (`"document"` | `"link"`), optional `fileName`, `s3Key` und `url`. Dateien landen je nach Backend in `brumm/documents/` (Drive, Lazy-Init via `ensureDocumentsFolder`) oder als S3-Objekt. API-Endpunkte: `GET/POST /api/ini/{slug}/documents`, `PATCH/DELETE /api/ini/{slug}/documents/{fileId}`, `PUT /api/ini/{slug}/documents/reorder`, `GET /api/ini/{slug}/documents/{fileId}/download` — Pfade bleiben `/documents`, nur die Frontend-Route heißt `/wall`. POST erkennt am Content-Type ob JSON (Link) oder Multipart (Datei). Dashboard-Card `DashboardWall` zeigt Einträge in `order`-Reihenfolge mit Links/Downloads sowie „Bearbeiten"-Button für SUPERUSER/MANAGER.
...
- **storageId vs. storageRef**: In der Datenbank wird nur die 8-stellige `storageId` (cuid2) gespeichert. Der vollständige `storageRef` (`YYYY-MM-DD-vorname-nachname_storageId`) existiert nur in Sheets und wird bei Bedarf daraus rekonstruiert
- **Dev-Fallback**: Solange `isSetupDone = false` werden Mitgliederdaten in `User.localData` (JSON-Feld in der Datenbank) zwischengespeichert. Nach dem Storage-Onboarding werden sie nach Sheets migriert und aus der Datenbank gelöscht
...
- **Invite-Logik**: Beim Anlegen eines Kindes kann `canManageMembers` per Checkbox „Eltern zum Unterlagen-Upload einladen" (default: aktiv) steuern, ob ein Invite-Link per E-Mail verschickt wird. Sind keine Vertragsvorlagen vorhanden, ist die Checkbox disabled und `sendInvite` wird automatisch auf `false` gesetzt; ein grauer Hinweis mit Link zu `/contract-templates` erscheint. Ist mindestens eine Guardian-Email identisch mit der Email des eingeloggten SUPERUSER (`hasSuperUserEmail`), wird sofort ein Invite mit `isUsed: true` erstellt → Kind startet mit `status = REGISTERED`. Ohne `hasSuperUserEmail` und mit aktivem `sendInvite` startet das Kind mit `status = PENDING_INVITE`. Beim Klick auf den Invite-Link setzt `verify/[token].get.ts` den Status auf `REGISTERED`. `parentAlreadyRegistered` wird nur gegen `inviteEmails` (nicht-Superuser-Emails) geprüft. `Member.hasInvite` (API-Feld, aus der `Invite`-Tabelle: `!!anyInvite`) zeigt ob je ein Invite für dieses Kind erstellt wurde — steuert welcher Workflow auf der Detailseite greift (Template-Liste vs. Direkt-Upload)
- **Magic-Link-Lookup**: 1. UserEmail in der Datenbank (deckt SUPERUSER ab), 2a. wenn Club `POSTGRES`-Backend nutzt: `findMemberIdByEmail` + `findManagerIdByEmailPg` + `findTeamMemberIdByEmailPg` parallel (Priorität MANAGER > TEAM > MEMBER), 2b. sonst: Managers-Sheet `findManagerIdByEmail` + Team-Sheet `findTeamMemberIdByEmail` + Members-Sheet `findUserIdByEmail` parallel (wenn Setup fertig) – selbe Prioritätsreihenfolge; bei Manager-/Team-Treffer wird User via `storageId` gesucht, 3. `Manager.localData` + `User.localData`-Suche in der Datenbank (Dev-Fallback, Priorität MANAGER > MEMBER). Existiert die userId nicht mehr in der Datenbank, wird kein Link erzeugt (stilles Fehlschlagen)
...
- **DSGVO-Cleanup** (`netlify/functions/cleanup.ts`): Läuft täglich als Netlify Scheduled Function. Löscht abgemeldete Kinder (`status = DEACTIVATED`, `deactivatedAt <= 1 Jahr`) nach 1 Jahr vollständig — sowohl DB-Einträge (User, UserEmail, Sessions, MagicLinks, Invites, MemberDocuments) als auch Drive-Ordner (Suche per `storageId`) und Sheets-Zeile (Suche per `userId` in Spalte A) bzw. Postgres-Zeile beim POSTGRES-Backend. Query ist explizit auf `role: 'MEMBER'` beschränkt. Löscht zusätzlich alle abgelaufenen `DeviceSession`-Einträge (`expiresAt < now`).
...
- **Netlify**: `NETLIFY_NEXT_PLUGIN_SKIP = "true"` in `netlify.toml` verhindert, dass der global installierte `@netlify/plugin-nextjs` beim Nuxt-Build fälschlicherweise ausgeführt wird. `netlify/functions/deploy-succeeded.ts` läuft nach jedem erfolgreichen Deployment als Event-Handler und führt ausstehende SQL-Migrationen auf allen Club-Datenbanken mit `type = POSTGRES` durch. Ergebnis (angewendete Migrations oder Fehler) wird als `AuditLog`-Eintrag gespeichert (`event: DEPLOY_MIGRATION_DONE` / `DEPLOY_MIGRATION_FAILED`, `severity: INFO` / `ERROR`)
...
| Datenmigration (Sheets→Postgres, Postgres→Postgres) | `server/utils/dataTransfer.ts` |
| Dateiübertragung (Drive→S3) | `server/utils/fileTransfer.ts` |
...
| Settings API (Datenbank-Backend) | `server/api/ini/[slug]/settings/database.*`, `database/transfer*` |
| Settings API (Datei-Storage-Backend) | `server/api/ini/[slug]/settings/file-storage.*`, `file-storage/transfer*` |
