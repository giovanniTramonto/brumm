...
- **filteredDocuments**: Computed in `pages/ini/[slug]/members/[id]/index.vue` — Drive-Dateien aus `documents/`, die nicht bereits als Template-Einreichung (`MemberDocument.driveFileId`) erfasst sind. Wird in der `canManageMembers`-Sektion nur angezeigt wenn `member.status === 'ACTIVE' || member.status === 'INACTIVE'` (verhindert Doppeldarstellung im Kein-Invite-Workflow vor Freischaltung).
