...
- **Einreichen-Flow**: MEMBER klickt „Einreichen" sobald alle Vertragsunterlagen vollständig sind. `User.hasSubmittedDocuments` wird auf `true` gesetzt (Neon). Benachrichtigung geht an: MANAGERs mit `isMemberManager = true` (Emails aus Sheets), oder – falls keine solchen existieren – an den SUPERUSER. Der „Freischalten"-Button ist erst aktiv wenn `hasSubmittedDocuments = true`; neben dem Titel „Vertragsunterlagen" erscheint ein Hinweis solange noch nicht eingereicht wurde.
