...
- **Dashboard-Komponenten**: Das Dashboard ist in wiederverwendbare Komponenten unter `components/dashboard/` aufgeteilt: `DashboardStats` (Kinder-Zahlen), `DashboardMemberStatus` (Anmeldungs-Card für MEMBER, zeigt Kinderliste mit Status — kein Link zur Kinderliste), `DashboardAktuell` (Links wie Adressliste), `DashboardDocuments` (Vereinsunterlagen-Card, fetcht Daten selbst).
